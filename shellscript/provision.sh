...
while [ -z "$_assembly" ]; do
  read -p "Input assembly (peer, api, dual): " _assembly
  case $_assembly in
    "peer")
    ;;
    "api")
    ...
    *)
      _assembly=""
      echo "Please input one of *peer, api, dual*"
    ;;
  esac
done

read -p "Input FriendlyName: " _friendlyName

read -p "Input Host(IP Address or FQDN): " _host

while [ -z "$_ready" ]; do
  echo ""
  echo "----------------------------"
  echo "Role: $_assembly"
  echo "FriendlyName: $_friendlyName"
  echo "Host: $_HOST"
  echo "----------------------------"
  echo "Ready? (Y/N)"
  read -n1 _ready
...
# echo '{"log-driver":"json-file","log-opts":{"max-size":"500k","max-file":"3"}}' > /etc/docker/daemon.json
...
check host localhost with address localhost
    ...
    if failed port 3000 protocol http
        ...
        and within 3 cycles
        then restart
    if failed port 7900 within 3 cycles
        then restart
    if failed port 7902 within 3 cycles
        then restart
__EOD__
